...
func initCaassandra() {
	...
	if err := Cassandra.Session.Query(`CREATE KEYSPACE IF NOT EXISTS quillpen 
	...
	if err := Cassandra.Session.Query(`CREATE TABLE IF NOT EXISTS quillpen.accounts ( 
	...
	if err := Cassandra.Session.Query(`CREATE TABLE IF NOT EXISTS quillpen.messages (
	...
	if err := Cassandra.Session.Query(`CREATE TABLE IF NOT EXISTS quillpen.users ( 
	...
	// conversations
	if err := Cassandra.Session.Query(`CREATE TABLE IF NOT EXISTS quillpen.conversations ( 
		conversation_id UUID,
		friend_id UUID,
		friend_name TEXT,
		user_id UUID ,
	...
	if err := Cassandra.Session.Query(`CREATE TABLE IF NOT EXISTS quillpen.conversations_by_participants ( 
		conversation_id UUID,
